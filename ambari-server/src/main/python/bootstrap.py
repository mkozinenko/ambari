...
class BootstrapDefault(Bootstrap):
    ...
    def resolveAgent(self, expected_hostname):
        from socket import gethostbyaddr
        self.host_log.write("Resolving host: {}\n".format(self.host))
        for loop in range(6):
            try:
                resolved_hostname,_,_  = gethostbyaddr(expected_hostname)
            except Exception:
                resolved_hostname = self.host
                self.host_log.write("Traceback: " + traceback.format_exc())
                self.host_log.write("### loop: {}".format(loop + 1))
                time.sleep(10)
        return self.stripConsulDomain(resolved_hostname)

    def stripConsulDomain(self, host):
        s = host.split('.')
        if len(s) > 3 and s[:len(s) - 4:-1][0] == 'consul' and s[:len(s) - 4:-1][2] == 'node':
            return '.'.join(s[:len(s) - 3:])
        return host
    ...
    def runSetupAgent(self):
        ...
        # ToDo: need resolve self.host to FQDN
        # command = self.getRunSetupCommand(self.host)
        expected_hostname = self.resolveAgent(self.host)
        command = self.getRunSetupCommand(expected_hostname)
        ssh = SSH(params.user, params.sshkey_file, self.host, command,
                  params.bootdir, self.host_log)
        retcode = ssh.run()
        self.host_log.write("\n")
    ...
    def saveConsulConf(self, target, bind_addr, join_addr, is_server=False):
        ...
        with open(target, 'w') as outfile:
            json.dump(consulConf, outfile, indent=4, separators=(',', ': '))
            outfile.close()

    def saveConsulService(self, target):
    ...
    def deployPackages(self):
        params = self.shared_state
        common_packages = ['nc','ntp']
        java_command = "mkdir -p /usr/jdk64 && cd /usr/jdk64 && wget -qO- http://public-repo-1.hortonworks.com/ARTIFACTS/jdk-8u60-linux-x64.tar.gz | tar -zx"
        selinux_command = "sed -i 's/=enforcing/=disabled/g' /etc/selinux/config && setenforce 0 || echo 1"
        command = "sudo bash -c 'yum -y install "+' '.join(common_packages)+" && systemctl enable ntpd && systemctl start ntpd && "+java_command+" && "+selinux_command+"'"
        ssh = SSH(params.user, params.sshkey_file, self.host, command, params.bootdir, self.host_log)
        retcode = ssh.run()
        return retcode["exitstatus"]

    def getBindAddrPair(self):
        params = self.shared_state
        command = "echo -n $SSH_CONNECTION | awk '{ print $1\" \"$3 }' | tr '\n' ' '"
        ssh = SSH(params.user, params.sshkey_file, self.host, command, params.bootdir, self.host_log)
        return ssh.run()["log"].strip().split(' ')[:-4]

    def deployHelpers(self):
        params = self.shared_state
        command = "cd / ; curl http://128.107.33.156/repo/helpers.tar.gz | sudo tar xzf -"
        ssh = SSH(params.user, params.sshkey_file, self.host, command, params.bootdir, self.host_log)
        retcode = ssh.run()
        return retcode["exitstatus"]
    ...
    def deployConsul(self):
        ...
        params = self.shared_state
        self.host_log.write("==========================\n")
        self.host_log.write("Prepare for Consul deploy procedure...")

        (join_addr, bind_addr) = self.getBindAddrPair()
        self.host_log.write("join_addr: {}, bind_addr: {}".format(join_addr, bind_addr))

        if join_addr == bind_addr:
            command = "echo \"" + \
            ...
            ssh = SSH(params.user, params.sshkey_file, self.host, command, params.bootdir, self.host_log)
            retcode = ssh.run()
            self.host_log.write("Skipping installation of Consul on Ambari server, just fixing resolv.conf")
            return retcode["exitstatus"]
        ...
        command = "pushd /etc/yum.repos.d &&" \
                  ...
                  "popd && " \
                  "sudo yum install -y consul consul-ui"
        ssh = SSH(params.user, params.sshkey_file, self.host, command,
                  params.bootdir, self.host_log)
        retcode1 = ssh.run()
        ...
        command = "sudo cp {} /etc/consul/consul.json  && sudo chown consul:consul /etc/consul/consul.json".format(tmpConsulConfig)
        ssh = SSH(params.user, params.sshkey_file, self.host, command,
                  params.bootdir, self.host_log)
        retcode3 = ssh.run()
        ...
        command = "echo \"" + \
                  ...
                  "server=/consul/{}/127.0.0.1#8600\n".format(self.reverse_net_ptr(bind_addr)) + \
                  "\" > /tmp/dnsmasq.d_10-consul.tmp && " + \
                  "sudo cp /tmp/dnsmasq.d_10-consul.tmp /etc/dnsmasq.d/10-consul"
        ssh = SSH(params.user, params.sshkey_file, self.host, command,
                  params.bootdir, self.host_log)
        retcode4 = ssh.run()

        # TODO: Will parse nameservers from /etc/resolv.conf and append to /etc/dnsmasq.d/00-default file
        command = "echo \"" + \
                  "server=8.8.8.8\n" + \
                  "server=8.8.4.4\n" + \
                  "\" > /tmp/dnsmasq.d_00-default.tmp && " + \
                  "sudo cp /tmp/dnsmasq.d_00-default.tmp /etc/dnsmasq.d/00-default"
        ssh = SSH(params.user, params.sshkey_file, self.host, command,
                  params.bootdir, self.host_log)
        retcode5 = ssh.run()

        command = "( cat /etc/NetworkManager/NetworkManager.conf ; echo \"" + \
                  "dns=none\n" + \
                  "\") > /tmp/NetworkManager.conf.tmp && " + \
                  "sudo cp /tmp/NetworkManager.conf.tmp /etc/NetworkManager/NetworkManager.conf"
        ssh = SSH(params.user, params.sshkey_file, self.host, command,
                  params.bootdir, self.host_log)
        retcode6 = ssh.run()

        command = "sudo find /etc/sysconfig/network-scripts/ifcfg-eth? " + \
        ...
        command = "( cat /etc/dnsmasq.conf | sed \'/^interface/d;/^listen_address/d;/^listen-address/d\' ; echo \"" + \
                  "listen-address={}\n".format(bind_addr) + \
                  "\") > /tmp/dnsmasq.conf.tmp && " + \
                  "sudo cp /tmp/dnsmasq.conf.tmp /etc/dnsmasq.conf && " + \
                  "sudo service dnsmasq restart && sleep 15"
        ssh = SSH(params.user, params.sshkey_file, self.host, command,
                  params.bootdir, self.host_log)
        retcode9 = ssh.run()

        # Fix /etc/hosts file
        command = "sudo sed -i -e \"/`hostname`/c{}\\t`hostname`\\t`hostname -s`\" /etc/hosts".format(bind_addr) + \
                  " && " + \
        ...
        command = "sudo systemctl stop consul.service && " \
        ...
        # sleep while restart services
        time.sleep(15)

        return max(retcode1["exitstatus"], retcode2["exitstatus"], retcode3["exitstatus"],
                   retcode4["exitstatus"], retcode5["exitstatus"], retcode6["exitstatus"],
                   retcode7["exitstatus"], retcode8["exitstatus"], retcode9["exitstatus"],
                   retcode10["exitstatus"], retcode11["exitstatus"])


    def updateLibRequests(self):
        params = self.shared_state
        self.host_log.write("==========================\n")
        self.host_log.write("Update the requests library...")
