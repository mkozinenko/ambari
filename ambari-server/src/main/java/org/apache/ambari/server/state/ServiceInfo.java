...
public class ServiceInfo implements Validable {
    ...
    public Map<String, QuickLinksConfigurationInfo> getQuickLinksConfigurationsMap() {
        ...
        try {
            LOG.info("quickLinksConfigurations -> " + quickLinksConfigurations);
        } catch (NullPointerException e) {
        }
        if (quickLinksConfigurations != null) {
            for (QuickLinksConfigurationInfo quickLinksConfiguration : quickLinksConfigurations) {
                LOG.info("for quickLinksConfiguration -> " + quickLinksConfiguration);
//
                tmp.remove(quickLinksConfiguration.getFileName());
                LOG.info("tmpAfterDelete -> " + tmp.get(quickLinksConfiguration.getFileName()));
                DynamicQuickLinks consulQuickLinks = new DynamicQuickLinks();
                QuickLinksConfiguration qlConf = new QuickLinksConfiguration();
                qlConf.setLinks(consulQuickLinks.getQuickLinks(quickLinksConfiguration.getConsulUrl(), quickLinksConfiguration.getConsulKey()).getLinks());
//
                QuickLinks ql = new QuickLinks();
                ql.setQuickLinksConfiguration(qlConf);
                Map<String, QuickLinks> mapQl = new HashMap<String, QuickLinks>();
                mapQl.put("QuickLinksConfiguration", ql);
                quickLinksConfiguration.setQuickLinksConfigurationMap(mapQl);

                tmp.put(quickLinksConfiguration.getFileName(), quickLinksConfiguration);
                LOG.info("tmpAdded -> " + tmp.get(quickLinksConfiguration.getFileName()));
            }
        }
